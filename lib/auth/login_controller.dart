...
class LoginController {
  ...
  Future<void> login(String email, String password, GlobalKey<FormState> formKey, Function(bool) setLoading) async {
    ...
    print("🔵 [DEBUG] Tentative de connexion avec: $email");

    try {
      final user = await api.login(email.trim(), password.trim());

      print("✅ [DEBUG] Connexion réussie: ${user.email}");
      ...
      print("❌ [DEBUG] Erreur lors de la connexion: $e");
      String errorMessage = e.toString();
      ...
      if (errorMessage.contains("403") ||
          errorMessage.toLowerCase().contains("vérifier votre email")) {
        print("⚠️ [DEBUG] Triggering email verification dialog");
        AwesomeDialog(
          context: context,
          dialogType: DialogType.warning,
          animType: AnimType.bottomSlide,
          ...
          btnOkOnPress: () async {
            try {
              print("🔵 [DEBUG] Renvoyer email de vérification...");
              final message = await api.resendVerificationEmail(email.trim());
              print("✅ [DEBUG] Email verification resent: $message");
              AwesomeDialog(
                context: context,
                dialogType: DialogType.success,
                animType: AnimType.bottomSlide,
        ...
        print("⚠️ [DEBUG] Showing generic error dialog");
        AwesomeDialog(
          context: context,
          dialogType: DialogType.error,
          animType: AnimType.bottomSlide,
